...
void BPlusTree::DestroyChilds(page_id_t node_pid) {
...
bool BPlusTree::GetValue(const GenericKey *key, std::vector<RowId> &result, Transaction *transaction) {
  //return false;
    ...
    Page* page = FindLeafPage(keytype.first,keytype.second, false);
...
BPlusTreeInternalPage *BPlusTree::Split(InternalPage *node, Transaction *transaction) {
  //return nullptr;
    ...
    InternalPage* internalnode = node;
...
template <typename T>
bool BPlusTree::Coalesce(T*& neighbor_node, T*& node, InternalPage*& parent, int index,
    Transaction* transaction) {
    if ((node)->IsLeafPage()) {
        LeafPage* leaf_node = reinterpret_cast<LeafPage*>(node);
        LeafPage* leaf_neighbor = reinterpret_cast<LeafPage*>(neighbor_node);
        if (index == 0 || (*parent).ValueIndex((node)->GetPageId()) <
            (*parent).ValueIndex((neighbor_node)->GetPageId()))  //first node, to right
        {
...
void BPlusTree::Redistribute(LeafPage *neighbor_node, LeafPage *node, int index) {
    if (index == 0) {
        LeafPage* leafnode = node;
        LeafPage* leafnei = neighbor_node;
...
void BPlusTree::Redistribute(InternalPage *neighbor_node, InternalPage *node, int index) {
    if (index == 0)
    {
        InternalPage* leafnode = node;
        InternalPage* leafnei = neighbor_node;
...
IndexIterator BPlusTree::Begin() {
  ...
  Page* page = FindLeafPage(keytype.first,keytype.second, true);
...
IndexIterator BPlusTree::End() {
  //return IndexIterator();
    ...
    return IndexIterator(page_leaf->GetPageId(), buffer_pool_manager_,page_leaf->GetSize() - 1);
}
